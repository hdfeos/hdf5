...
main(void)
{
    ...
    /* MSC 
     *need more work for cross endian machines
     * 1) types need to be endian specific (NATIVE won't work)
     * 2) fill values need to be decoded with endianess in mind
    ...
    */
    ...
    if((ret = encode_plist(dcpl1, little_endian, "plist_files/dcpl_le", "plist_files/dcpl_be")) < 0)
        assert(ret > 0);
    ...
    if((ret = encode_plist(dapl1, little_endian, "plist_files/dapl_le", "plist_files/dapl_be")) < 0)
        assert(ret > 0);
    ...
    if((ret = encode_plist(dxpl1, little_endian, "plist_files/dxpl_le", "plist_files/dxpl_be")) < 0)
        assert(ret > 0);
    ...
    if((ret = encode_plist(gcpl1, little_endian, "plist_files/gcpl_le", "plist_files/gcpl_be")) < 0)
        assert(ret > 0);
    ...
    if((ret = encode_plist(lcpl1, little_endian, "plist_files/lcpl_le", "plist_files/lcpl_be")) < 0)
        assert(ret > 0);
    ...
    if((ret = encode_plist(ocpl1, little_endian, "plist_files/ocpl_le", "plist_files/ocpl_be")) < 0)
        assert(ret > 0);
    ...
    if((ret = encode_plist(lapl1, little_endian, "plist_files/lapl_le", "plist_files/lapl_be")) < 0)
        assert(ret > 0);
    ...
    if((ret = encode_plist(fapl1, little_endian, "plist_files/fapl_le", "plist_files/fapl_be")) < 0)
        assert(ret > 0);
   ...
   if((ret = H5Pset_shared_mesg_phase_change(fcpl1, 60, 20) < 0))
    ...
    if((ret = encode_plist(fcpl1, little_endian, "plist_files/fcpl_le", "plist_files/fcpl_be")) < 0)
        assert(ret > 0);
