 ...
 */
static herr_t
H5B2__split1(H5B2_hdr_t *hdr, hid_t dxpl_id, uint16_t depth, H5B2_node_ptr_t *curr_node_ptr,
...
/* Actually, this can be easily updated (for 2-node redistrib.) and shouldn't require re-searching */
    ...
    if(depth > 1) {
        if(H5B2__insert_internal(hdr, dxpl_id, (uint16_t)(depth - 1), &internal_flags, &internal->node_ptrs[idx], udata) < 0)
            HGOTO_ERROR(H5E_BTREE, H5E_CANTINSERT, FAIL, "unable to insert record into B-tree internal node")
    } /* end if */
    else {
        if(H5B2__insert_leaf(hdr, dxpl_id, &internal->node_ptrs[idx], udata) < 0)
...
/* Actually, this can be easily updated (for 2-node redistrib.) and shouldn't require re-searching */
    ...
    if(depth > 1) {
        ...
        if(H5B2__remove_leaf(hdr, dxpl_id, new_node_ptr, udata, op, op_data) < 0)
    ...
    void *op_data)
{
    ...
    if(depth > 1) {
        ...
        if(H5B2__remove_leaf_by_idx(hdr, dxpl_id, new_node_ptr, (unsigned)n, op, op_data) < 0)
            HGOTO_ERROR(H5E_BTREE, H5E_CANTDELETE, FAIL, "unable to remove record from B-tree leaf node")
    } /* end else */
