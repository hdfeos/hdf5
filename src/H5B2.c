...
H5B2_insert(H5B2_t *bt2, hid_t dxpl_id, void *udata)
{
    ...
    if(hdr->depth > 0) {
        if(H5B2__insert_internal(hdr, dxpl_id, hdr->depth, NULL, &hdr->root, udata) < 0)
            HGOTO_ERROR(H5E_BTREE, H5E_CANTINSERT, FAIL, "unable to insert record into B-tree internal node")
    } /* end if */
    else {
        if(H5B2__insert_leaf(hdr, dxpl_id, &hdr->root, udata) < 0)
    ...
    void *op_data)
{
    ...
    if(hdr->depth > 0) {
        ...
        if(H5B2__remove_leaf(hdr, dxpl_id, &hdr->root, udata, op, op_data) < 0)
    ...
    hsize_t idx, H5B2_remove_t op, void *op_data)
{
    ...
    if(hdr->depth > 0) {
        ...
        if(H5B2__remove_leaf_by_idx(hdr, dxpl_id, &hdr->root, (unsigned)idx, op, op_data) < 0)
            HGOTO_ERROR(H5E_BTREE, H5E_CANTDELETE, FAIL, "unable to remove record from B-tree leaf node")
    } /* end else */
